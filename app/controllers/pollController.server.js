...
module.exports = function(path, app_url) {
    ...
    /** Given a request, creates new poll based on data in the req.*/
    var createNewPollFromReq = function(req) {
        var user = req.user;
        var poll = new Poll();
	...
	this.addPoll = function(req, res) {
        if (req.user && req.body) {
            ...
            Poll.findOne({name: pollName}, function(err, result) {
                if (result) {
                    var msg = {msg: "Poll with name " + pollName + " already exists."};
                    return res.json(msg);
                }
                var poll = createNewPollFromReq(req);
                var user = req.user;
